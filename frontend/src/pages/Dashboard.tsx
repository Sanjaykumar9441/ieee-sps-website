...
const Dashboard = () => {
  ...
  const fetchRegistrationStatus = async () => {
    const res = await axios.get(
      `${import.meta.env.VITE_API_URL}/events/registration-status`,
    );
  ...
  const refreshPauseStatus = async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin-access/profile`,
        {
          headers: {
  ...
  const handleUpdateMember = async (member: any) => {
    ...
    await axios.put(
      `${import.meta.env.VITE_API_URL}/team/${member._id}`,
      formData,
      { headers: { Authorization: `Bearer ${token}` } },
    );
  ...
  const menu = isPaused
    ...
    : [
        ...
        ...(permissions.assessmentPlatform
          ? [
              {
                id: "assessmentDashboard",
  ...
  const refreshPermissions = async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin-access/permissions`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "#FAF9F7", color: "#1C1B22" }}
    >
      {/* HEADER */}
      <header
      ...
      >
        ...
        <button
          ...
          onMouseEnter={(e) => {
            (e.currentTarget as HTMLElement).style.backgroundColor = "#FBD5D5";
          }}
          onMouseLeave={(e) => {
            (e.currentTarget as HTMLElement).style.backgroundColor = "#FDEBEB";
          }}
        >
          <LogOut size={15} />
          Logout
      ...
      <div className="flex flex-1 relative">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to bottom, #8B7FF5, #6C5FE0, transparent)",
            }}
          />
            ...
            {menu.map((item) => {
              ...
              return (
                <button
                  ...
                  style={{
                    backgroundColor: isActive ? "#7C6FEF" : "transparent",
