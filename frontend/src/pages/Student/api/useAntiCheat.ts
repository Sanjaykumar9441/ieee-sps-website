...
const getSessionHeaders = (attemptId: string) => ({
...
export default function useAntiCheat({
  ...
  const loadInfractions = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        ...
        {
          headers: getSessionHeaders(attemptId),
        },
      );

      const count = Number(
        data?.count ?? data?.totalInfractions ?? data?.infractions?.length ?? 0,
      );

      setInfractionCount(Math.max(0, count));
    } catch (error) {
  ...
  const reportInfraction = useCallback(
    async (type: InfractionType) => {
      if (!attemptId || !enabled || autoSubmitRef.current) {
        return;
      }
      ...
      try {
        const { data } = await axios.post<InfractionResponse>(
          `${API}/api/student-assessments/${attemptId}/infractions`,
          {
            type,
            metadata: {
        ...
        const serverCount = data?.totalInfractions ?? data?.count;

        const count = Number(serverCount ?? infractionCount + 1);

        setInfractionCount(Math.max(0, count));

        if (
          data?.autoSubmitted ||
          data?.autoSubmitRequired ||
        ...
        if (
          ...
          onAutoSubmit?.("ALREADY_SUBMITTED");
        }
      }
    },
  ...
  useEffect(() => {
    if (!attemptId || !enabled || !observeBrowserEvents) {
      return;
    }
