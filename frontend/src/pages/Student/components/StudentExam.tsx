...
export default function StudentExam({
  ...
  const submitForReason = useCallback(
    async (reason: SubmitReason = "STUDENT_SUBMIT") => {
      ...
      try {
        ...
        const result = await submitAssessment(attemptId, reason);
    ...
    [attemptId, finishExam, persistCurrentAnswer],
  );
  ...
  const { connected, reconnecting, reconnectCount } = useExamSocket({
    ...
    onResync: (data: any) => {
      ...
      if (typeof data.remainingSeconds === "number") {
        const server = Math.max(0, Number(data.remainingSeconds));
        const local = Math.max(
          0,
          Math.ceil((deadlineRef.current - Date.now()) / 1000),
        );
        if (Math.abs(server - local) > 2) {
          deadlineRef.current = Date.now() + server * 1000;
    ...
    const sync = async () => {
      ...
      try {
        ...
        if (typeof data.remainingSeconds === "number") {
          ...
          if (server <= 0) void submitRef.current("AUTO_SUBMIT");
        }
        if (Array.isArray(data.palette)) setPalette(data.palette);
      } catch (error) {
  ...
  const loadQuestion = useCallback(
    async (number: number) => {
      ...
      try {
        ...
        const server = Math.max(0, Number(result.remainingSeconds || 0));
        const local = Math.max(
          0,
          Math.ceil((deadlineRef.current - Date.now()) / 1000),
        );
  ...
  const handleNext = async () => {
    ...
    if (saved && currentQuestion < totalQuestions)
      await loadQuestion(currentQuestion + 1);
  };
  const handlePrevious = async () => {
    if (currentQuestion <= 1) return;
    const saved = await persistCurrentAnswer();
    if (saved) await loadQuestion(currentQuestion - 1);
  };
  const handlePaletteSelect = async (number: number) => {
    const saved = await persistCurrentAnswer();
    if (saved) await loadQuestion(number);
  };
  useEffect(() => {
    const visibility = () => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_330px] gap-6">
              ...
              <aside className="space-y-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">
                        {connected
                          ? reconnectCount
                            ? `Connection restored • ${reconnectCount} reconnect${reconnectCount !== 1 ? "s" : ""}`
                ...
                <button
                  ...
                  disabled={submitting}
                  className="w-full h-12 rounded-xl bg-[#00629B] text-white font-semibold flex items-center justify-center gap-2 hover:bg-[#00527f]"
                >
                  <Send size={17} />
                  Submit Assessment
                </button>
