...
interface Props {
  ...
  onLoggedIn?: () => void;
  onOtpSent?: (studentEmail: string) => void;
  loginMethod?: "PASSWORD" | "OTP";
}

export default function StudentLogin({ assessmentId, assessmentTitle, onLoggedIn, onOtpSent, loginMethod = "PASSWORD" }: Props) {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setLoading(true);
      await requestStudentOtp(assessmentId, normalizedEmail);
      setOtpSent(true);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (loginMethod === "OTP" && !otpSent) return toast.error("Send the OTP first.");
    try {
      setLoading(true);
  ...
  return <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4"><div className="w-full max-w-md"><div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
    ...
    <form onSubmit={handleSubmit} className="p-8"><div className="mb-7"><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Assessment</p><h2 className="mt-1 text-lg font-semibold text-slate-900">{assessmentTitle}</h2></div>
      ...
      {loginMethod === "OTP" && <button type="button" disabled={loading || !normalizedEmail} onClick={handleSendOtp} className="mt-4 w-full rounded-xl border border-[#00629B] px-4 py-3 font-semibold text-[#00629B] disabled:opacity-50">{otpSent ? "Resend OTP" : "Send OTP"}</button>}
