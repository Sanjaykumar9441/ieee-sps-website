...
const AdminsTab = () => {
  ...
  const deleteAdmin = async (id: string) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/admin-access/${id}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      fetchAdmins();
  ...
  const resetPassword = async (id: string) => {
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/admin-access/reset-password/${id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
      fetchAdmins();
  ...
  const toggleAdminStatus = async (id: string, currentlyPaused: boolean) => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/admin-access/toggle-status/${id}`,
        {
          reason,
        },
        {
  ...
  const saveAccess = async () => {
    try {
      if (editingAdminId) {
        await axios.put(
          `${import.meta.env.VITE_API_URL}/api/admin-access/${editingAdminId}`,
          {
            permissions,
          },
          {
