...
interface Props {
  ...
  onAssessmentDataChanged?: () => void;
}

export default function QuestionBanks({
  assessment,
  onAssessmentDataChanged,
}: Props) {
  ...
  const fetchBanks = async () => {
    try {
      setLoading(true);
      setBanks((await getQuestionBanks(assessment.id)) || []);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm(
        "Delete this Question Bank? Its questions will also be deleted permanently.",
      )
    )
      return;
    try {
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-500">
            Build reusable MCQ, multiple-correct, true/false and
            fill-in-the-blank banks.
          </p>
        </div>
        <button
          type="button"
          ...
          className="flex items-center justify-center gap-2 rounded-xl bg-[#00629B] px-5 py-3 font-semibold text-white shadow-sm hover:bg-[#00527f]"
        >
      ...
      <div className="flex items-center gap-3 rounded-2xl border bg-white p-4 shadow-sm">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 py-3 pl-11 pr-4 outline-none focus:border-[#00629B]"
          />
        </div>
        <div className="hidden rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-500 sm:block">
      ...
      {filteredBanks.length === 0 ? (
        <div className="rounded-2xl border bg-white p-14 text-center">
          ...
          <h3 className="mt-5 text-xl font-semibold text-slate-900">
            {search ? "No Question Banks Found" : "No Question Banks Yet"}
          </h3>
          <p className="mt-2 text-sm text-slate-500">
            {search
              ? "Try a different search term."
              : "Create your first bank and add questions manually or by CSV."}
          </p>
        </div>
      ) : (
