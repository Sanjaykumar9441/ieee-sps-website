...
const isSubmitted = (s: LiveStudent) =>
  normalizeStatus(s.status) === "SUBMITTED" || Boolean(s.isExpired);
const isLive = (s: LiveStudent) =>
  !isSubmitted(s) &&
  ["IN_PROGRESS", "LIVE", "STARTED"].includes(normalizeStatus(s.status));
...
export default function LiveMonitor({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Unable to load live students.",
      );
    } finally {
      setLoading(false);
    }
    ...
    ? Math.round(
        students.reduce(
          (sum, s) =>
            sum +
            (s.totalQuestions
              ? Math.min(100, (s.answeredQuestions / s.totalQuestions) * 100)
              : 0),
          0,
        ) / students.length,
      )
                ...
                filtered.map((student) => {
                  ...
                  return (
                    <tr key={student.attemptId} className="text-sm">
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${student.violations ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-600"}`}
                        >
                          {student.violations}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-bold ${submitted ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}
                        >
