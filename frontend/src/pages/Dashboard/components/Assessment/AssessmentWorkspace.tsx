...
export default function AssessmentWorkspace({ assessment, onClose }: Props) {
  ...
  return (
    <div className="assessment-workspace-premium mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
      <header className="bg-slate-950 px-6 py-6 text-white md:px-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <span
                className={`rounded-full px-3 py-1 text-xs font-semibold ${assessment.is_published ? "bg-emerald-500/20 text-emerald-300" : "bg-white/10 text-slate-300"}`}
              >
                {status}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-300">
                {assessment.login_method === "OTP"
                  ? "OTP Login"
                  : "Password Login"}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-slate-300">
          ...
          <button
            type="button"
            onClick={onClose}
            className="self-start rounded-xl border border-white/15 p-2.5 text-slate-300 hover:bg-white/10"
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
          <MiniStat
            label="Questions"
            value={String(assessment.total_questions || 0)}
          />
          <MiniStat
            label="Duration"
            value={`${assessment.duration_minutes || 0} min`}
          />
