...
const RoleChip = ({ name, subtitle }: { name?: string; subtitle?: string }) => (
  <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full pl-1 pr-3 py-1">
    <span className="w-6 h-6 rounded-full bg-[#00629B]/30 text-[10px] font-semibold text-cyan-200 flex items-center justify-center">
      {initials(name)}
    </span>
  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl p-6" style={cardStyle}>
        ...
        <p className="text-slate-400 mt-2">Registration progress of IEEE SPS events.</p>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          <div className="rounded-2xl bg-white/5 border border-white/10 p-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">{ARDUINO_STATS.eventName}</h3>
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 p-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">Membership Drive</h3>
      ...
      <div className="rounded-2xl p-6" style={cardStyle}>
        ...
        <p className="text-slate-400 mt-2">Committee member distribution by role.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
          ...
          <div className="rounded-xl bg-white/5 border border-white/10 p-5">
            <p className="text-slate-400 text-sm">Vice Chair</p>
          ...
          <div className="rounded-xl bg-white/5 border border-white/10 p-5">
            <p className="text-slate-400 text-sm">Treasurer</p>
            <h3 className="text-3xl font-bold text-pink-400 mt-2">{treasurers.length}</h3>
          </div>

          <div className="rounded-xl bg-white/5 border border-white/10 p-5">
            <p className="text-slate-400 text-sm">Webmaster</p>
            <h3 className="text-3xl font-bold text-violet-400 mt-2">{webmasters.length}</h3>
          </div>

          <div className="rounded-xl bg-white/5 border border-white/10 p-5 md:col-span-2 lg:col-span-3">
            <p className="text-slate-400 text-sm">Executive Members</p>
            <h3 className="text-3xl font-bold text-orange-400 mt-2">{executiveMembers.length}</h3>
          </div>
        </div>

        {/* Chair spotlight */}
        <div className="flex items-center justify-between py-4 mt-6 border-t border-white/10">
          <div className="flex items-center gap-3">
            <span className="w-10 h-10 rounded-full bg-[#00629B]/30 text-sm font-semibold text-cyan-200 flex items-center justify-center">
              {initials(chair?.name)}
            </span>
            <div>
              <p className="font-semibold text-white">{chair?.name || "Not assigned"}</p>
              <p className="text-sm text-slate-400">{chairSubtitle || "—"}</p>
            </div>
          </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <span className="font-semibold text-white">{executiveMembers.length}</span>
        </div>
