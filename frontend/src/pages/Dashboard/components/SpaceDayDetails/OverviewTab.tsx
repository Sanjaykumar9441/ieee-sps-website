  ...
  return (  
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">  
      {overview  
        ...
        .map((item) => (  
          <div key={item.label} className="rounded-2xl border border-[#EBE8E2] bg-[#FAF9F7] p-5">  
            <p className="text-sm text-[#8A8578]">{item.label}</p>

            <h3 className="mt-2 text-lg font-semibold text-[#1C1B22]">  
              {item.value || "-"}  
            </h3>  
          </div>  
        ))}  
