...
async function forceSubmitAttempt(attemptId, reason = "ADMIN_FORCE_SUBMIT") {
  ...
  if (activityError)
    console.warn("Could not record admin action:", activityError.message);
...
exports.forceSubmit = async (req, res) => {
  try {
    const result = await forceSubmitAttempt(
    ...
    return res.json({
      ...
      message: result.alreadyFinished
        ? "Assessment was already finished."
        : "Assessment force submitted successfully.",
...
exports.forceSubmitAll = async (req, res) => {
  try {
    ...
    for (const a of attempts || []) {
      try {
        await forceSubmitAttempt(a.id, "ADMIN_FORCE_SUBMIT_ALL");
        submitted++;
      } catch (e) {
        failed++;
        console.error("Force submit attempt failed:", a.id, e.message);
      }
    }
    await refreshLeaderboard(assessmentId);
    return res.json({
      success: true,
      message: `${submitted} assessment(s) force submitted.`,
      submitted,
      failed,
    });
  } catch (err) {
