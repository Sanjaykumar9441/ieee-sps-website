...
exports.getLiveStudents = async (req,res) => {
  try {
    ...
    const {data:attempts,error:attemptsError}=await supabase.from("assessment_attempts").select("id,assessment_id,student_id,started_at,submitted_at,expires_at,resumed_count,current_question,score,answered_questions,status").eq("assessment_id",assessmentId).order("started_at",{ascending:true});
    if(attemptsError)throw attemptsError;
    await reconcileExpiredAttempts(attempts);
    const {data:refreshedAttempts,error:refreshedError}=await supabase.from("assessment_attempts").select("id,assessment_id,student_id,started_at,submitted_at,expires_at,resumed_count,current_question,score,answered_questions,status").eq("assessment_id",assessmentId).order("started_at",{ascending:true});
...
exports.getStudentDetails=async(req,res)=>{
  try{
    ...
    const {data:attempt,error}=await supabase.from("assessment_attempts").select("id,assessment_id,student_id,started_at,submitted_at,expires_at,resumed_count,current_question,score,answered_questions,status").eq("id",attemptId).single();
    ...
    return res.json({success:true,student:student||null,attempt:{...attempt,startedAt:attempt.started_at,submittedAt:attempt.submitted_at,expiresAt:attempt.expires_at,resumedCount:Number(attempt.resumed_count||0),currentQuestion:Number(attempt.current_question||0),answeredQuestions:Number(attempt.answered_questions||0)},timeline:{loggedInAt:student?.first_login_at||null,startedAt:attempt.started_at,assessmentStartedAt:attempt.started_at,submittedAt:attempt.submitted_at},statistics:{questionsAnswered:Number(attempt.answered_questions||0),score:Number(attempt.score||0),violations:infractions?.length||0},infractions:infractions||[],activities:activities||[],questions:questions||[]});
